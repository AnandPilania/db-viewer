...
if (active.length === 0) {
    app.log.warn("No database drivers are installed. Install at least one, e.g.:\n" +
...
if (missing.length > 0) {
    app.log.info(`Drivers not installed (npm install @pilaniaanand/driver-<name> to enable): ${missing
        .map((d) => d.key)
        .join(", ")}`);
}
