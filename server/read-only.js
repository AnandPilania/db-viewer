 ...
 * replica.
 */
export function isReadOnlyConnection(config) {
...
export function assertWritable(config) {
    if (isReadOnlyConnection(config))
        throw new ReadOnlyError();
}
/** Call before execute() — only blocks if the query itself isn't a plain read (see isDestructiveExec). */
export function assertExecutable(config, query) {
    if (isReadOnlyConnection(config) && isDestructiveExec(query))
        throw new ReadOnlyError();
}
