        ...
        for (const [key, meta] of Object.entries(KNOWN_DRIVERS)) {
            const attempts = [
                { label: "node_modules", load: () => import(/* @vite-ignore */ meta.packageName) },
                {
                    ...
                    load: async () => {
                        ...
                        if (!entryUrl) {
                            const err = new Error(`Cannot find package '${meta.packageName}' under driversDir()`);
            ...
            for (const attempt of attempts) {
                try {
                    ...
                    const driver = mod[meta.exportName];
                    if (!driver) {
            ...
            if (!loaded) {
                const notFoundEverywhere = (lastError?.code === "ERR_MODULE_NOT_FOUND" || lastError?.code === "MODULE_NOT_FOUND") &&
