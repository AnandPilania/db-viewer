...
export async function exportRoutes(app) {
    app.get("/api/connections/:id/tables/:table/export", async (req, reply) => {
        ...
        const { schema, format = "csv" } = req.query;
        ...
        try {
            ...
            // Too late to change the HTTP status once streaming started — surface
            // the failure in-band so the download isn't silently truncated.
