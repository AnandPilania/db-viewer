...
describe("applyRowChange", () => {
    it("patches a single column by primary key", () => {
        const next = applyRowChange(rows(), { type: "update", primaryKey: { id: 2 }, column: "total", value: 99 }, [
            "id",
        ]);
    ...
    it("is a no-op when the value already matches (the client's own echo)", () => {
        const before = rows();
        const next = applyRowChange(before, { type: "update", primaryKey: { id: 1 }, column: "total", value: 10 }, [
            "id",
        ]);
    ...
    it("ignores an update for a row outside the loaded window", () => {
        const before = rows();
        expect(
            applyRowChange(before, { type: "update", primaryKey: { id: 999 }, column: "total", value: 1 }, ["id"])
        ).toBe(before);
    });
    ...
    it("refuses to guess when the primary key is only partially supplied", () => {
        ...
        expect(
            applyRowChange(compound, { type: "update", primaryKey: { id: 1 }, column: "v", value: "x" }, [
                "tenant",
                "id",
            ])
        ).toBe(compound);
