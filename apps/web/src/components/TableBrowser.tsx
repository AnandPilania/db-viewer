...
export function TableBrowser({ connectionId, table, initialFilter }: Props) {
    ...
    async function handleEditCell(rowIndex: number, column: ColumnDefinition, value: unknown): Promise<boolean> {
        ...
        const primaryKey = Object.fromEntries(columns.filter((c) => c.isPrimaryKey).map((c) => [c.name, row[c.name]]));
    ...
    // Both counts describe the whole table, so neither means anything once a
    // WHERE clause is in play — counting the filtered set exactly is the scan
    // this app exists to avoid. Filtered mode reports what's loaded instead.
    ...
    return (
        <div className="flex h-full flex-col">
              ...
              huge table without paging through everything before it. */}
                    ...
                    <div className="relative" onKeyDown={(e) => e.key === "Escape" && setExportOpen(false)}>
                        ...
                        {exportOpen && (
                            <div
                                role="menu"
                                className="absolute right-0 top-full z-20 mt-1 w-32 rounded-md border border-border bg-card shadow-lg"
                            >
                                <button
                                    role="menuitem"
            ...
            <FilterBar columns={columns} applied={filters} onApply={changeFilters} />
            ...
            {actionError && (
                <div
                    role="alert"
                    className="flex items-start justify-between gap-3 border-b border-border bg-destructive/10 px-3 py-2 text-xs text-destructive"
                >
            ...
            {windowCapped && (
                <div className="border-b border-border bg-muted/40 px-3 py-1.5 text-[11px] text-muted-foreground">
                    Showing a moving window of the most recent {rows.length.toLocaleString()} rows scrolled through.
                    Earlier rows are reloaded on scroll up
                    {seekColumn ? `, or jump straight to a ${seekColumn} value` : ""}.
                </div>
            )}
