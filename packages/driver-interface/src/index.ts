...
export interface ColumnDefinition {
    ...
    /** Postgres/MySQL/SQLite only — undefined elsewhere, treated as "no extra constraint". */
    maxLength?: number;
    enumValues?: string[];
    isUnique?: boolean;
    /** Raw CHECK clause text, display-only — never evaluated or parsed. */
...
/**
 * Checks that don't need a DB round trip: maxLength and enum membership.
 * Absent fields mean "no extra constraint" — Mongo/Redis/ClickHouse columns
 * never populate them, so this degrades to a no-op there automatically.
 */
export function checkConstraints(value: unknown, column: ColumnDefinition): string | null {
    if (value == null) return null;
    if (column.maxLength != null && typeof value === "string" && value.length > column.maxLength) {
        return `${column.name} must be at most ${column.maxLength} characters`;
    }
    if (column.enumValues?.length && !column.enumValues.includes(String(value))) {
        return `${column.name} must be one of: ${column.enumValues.join(", ")}`;
    }
    return null;
}
