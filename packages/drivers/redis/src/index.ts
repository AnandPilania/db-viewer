...
import { flattenFilters } from "@pilaniaanand/driver-interface";
import type {
    ColumnDefinition,
    ConnectionConfig,
    DatabaseDriver,
...
class RedisConnection implements DriverConnection {
    ...
    private async preview(key: string, type: RedisKeyType): Promise<{ size: number; value: unknown }> {
        switch (type) {
            case "string": {
                ...
                return {
                    size: len,
                    value: val.length > STRING_PREVIEW_CHARS ? val.slice(0, STRING_PREVIEW_CHARS) + "…" : val,
                };
            }
            case "hash": {
    ...
    async queryRows(options: QueryRowsOptions): Promise<QueryRowsResult> {
        ...
        // SCAN takes one glob and nothing else, so only a `key like` condition is
        // honoured — anywhere in the tree, AND/OR and all. Everything else is
        // dropped, which is why the UI only offers key filters for Redis.
        const patternFilter = flattenFilters(options.filters ?? []).find((f) => f.column === "key" && f.op === "like");
    ...
    private async ensureNotifying(): Promise<void> {
        ...
        this.notifySetupPromise = (async () => {
            ...
            await sub.pSubscribe("__keyevent@*__:*", async (key, channel) => {
                ...
                try {
                    const [ttl, { size, value }] = await Promise.all([
                        this.client.ttl(key),
                        this.preview(key, keyType),
                    ]);
                    const row = { key, ttl: ttl < 0 ? null : ttl, size, value };
                    for (const handler of handlers)
                        handler({ type: "update", primaryKey: { key }, column: "__row__", value: row });
                } catch {
